...
class DUCBBase(Bandit2D):
    ...
    def pick_pair(self, t):
        ...
        j_t = self.pick_jt(None, cb, i_t, zeta_cb, zeta)
...
class DUCB(DUCBBase):
    def pick_jt(self, r_hat, cb, i_t, zeta_cb, zeta):
        return np.argmax(zeta + 1 * cb[i_t, :])


class DUCBElim(DUCBBase):
    def __init__(self, model, seed, t_limit=1000):
        super().__init__(model, seed, t_limit)
        self.cI = set([i for i in range(self.n)])
    ...
    def pick_jt(self, r_hat, cb, i_t, zeta_cb, zeta):
        ...
        for i in range(self.n):
            for j in range(self.n):
                if u_zeta[i] < l_zeta[j]:
                    to_remove.add(i)
                    break
        self.cI = self.cI - to_remove
        if len(self.cI) == 1:
            return i_t
        else:
            return np.argmax(cb[i_t])
...
class DUCBBordaElim(Borda, DUCBElim):
    pass
...
'''
1. farzad said the compound confidence bound is not right
2. hao said we can try use the confidence bound similar to martin's paper
3. use linear bandit reduction
'''